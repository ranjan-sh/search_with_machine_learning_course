...
def search(client, user_query, index="bbuy_products", sort="_score", sortDir="desc"):
    ...
    cat_prediction = query_model.predict(user_query, threshold=0.5)
    filters = []
    if cat_prediction:
        categoryid = cat_prediction[0][0].removeprefix('__label__')
        # print(categoryid)
        filters.append({ "term": { "categoryPathIds": categoryid   }})
